...
export default function KontribusiSoalPage() {
  ...
  useEffect(() => {
    if (formData.kategori_soal_id) {
      fetchJenis(Number(formData.kategori_soal_id));
    } else {
      setJeniss([]);
      setFormData(prev => ({ ...prev, jenis_soal_id: '' }));
    }
  }, [formData.kategori_soal_id]);
  ...
  const validateForm = () => {
    if (!formData.kategori_soal_id) return 'Kategori Soal wajib dipilih';
    if (!formData.jenis_soal_id) return 'Jenis Soal wajib dipilih';
    if (!formData.level) return 'Level Soal wajib dipilih';
    if (formData.pertanyaan.length < 5) return 'Pertanyaan minimal 5 karakter';
    if (!formData.opsi_a || !formData.opsi_b || !formData.opsi_c || !formData.opsi_d || !formData.opsi_e) {
      return 'Semua opsi jawaban (A-E) wajib diisi';
    }
    if (!formData.jawaban_benar) return 'Jawaban benar wajib dipilih';
    return null;
  };
             ...
             <div className="grid md:grid-cols-3 gap-6">
               <div className="space-y-2">
                 ...
                 <select
                   ...
                   value={formData.kategori_soal_id}
                   onChange={handleInputChange}
                   className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl font-bold text-gray-700 outline-none transition-all appearance-none cursor-pointer"
                 >
               ...
               <div className="space-y-2">
                 ...
                 <select
                   name="jenis_soal_id"
                   value={formData.jenis_soal_id}
                   onChange={handleInputChange}
                   disabled={!formData.kategori_soal_id}
                   className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl font-bold text-gray-700 outline-none transition-all appearance-none cursor-pointer disabled:opacity-50"
                 >
                  ...
                  <div key={opt} className="space-y-3">
                    <div className="flex items-center gap-4">
                       <span className={clsx(
                         ...
                         formData.jawaban_benar === opt ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-400"
                       )}>
                         {opt}
                       </span>
                       <input
                         type="text"
                         name={`opsi_${opt.toLowerCase()}`}
                         value={(formData as any)[`opsi_${opt.toLowerCase()}`]}
                         onChange={handleInputChange}
                         placeholder={`Teks Opsi ${opt}`}
                         className="flex-1 px-5 py-4 bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl font-bold text-gray-700 outline-none transition-all"
                       />
                  ...
                  <div className="flex flex-wrap gap-3">
                    {OPSIS.map(opt => (
                      <button
                        key={opt}
                        type="button"
                        onClick={() => setFormData(prev => ({ ...prev, jawaban_benar: opt }))}
                        className={clsx(
